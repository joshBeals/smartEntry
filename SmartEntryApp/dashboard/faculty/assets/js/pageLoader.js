...
const numberAnalysis = () => {
	let token = getCookie('token');
	let fac = getCookie('fac');
    if(!token){
        window.location.replace("../../index.html");
	}
	document.querySelector('#welcome').innerHTML = `Welcome Back, ${fac}`;
	fetch('http://159.65.185.132:8888/smartentry/facultystudent', {
        method: 'POST',
        headers: {
